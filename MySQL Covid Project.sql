...
concat((sum(new_deaths)*100/max(total_cases)),"%") as death_rate_percentage 
from covid_deaths 
where location = 'ghana';

-- 2. Highest Infection Rate for all countries

select row_number() over (order by (max(total_cases)/max(population))*100 desc) as row_num, 
location, max(total_cases) as no_of_cases, 
max(population) as total_population, 
concat((max(total_cases)*100/max(population)),"%") as HighestInfectionRate 
from covid_deaths
...
select row_number() over (order by (max(total_deaths)/max(population))*100 desc) as row_num, 
location, max(total_deaths) as no_of_deaths, 
max(population) as total_population, 
concat((max(total_deaths)*100/max(population)),"%") as DeathRate 
from covid_deaths
where location not in ('asia','africa','europe','North america','european union','international','south america')
group by location;

-- 4. Finding death rate per population by continent

select continent, sum(new_deaths) as no_of_deaths, 
sum(distinct(population)) as total_population, 
concat((sum(new_deaths)*100/sum(distinct(population))),"%") as continental_death_rate 
from covid_deaths 
where location not in ('asia','africa','europe','North america','european union','international','south america')
group by continent;

-- 5. Finding infection rate per population by continent
...
-- Finding the death per infection rate per continent;
...
-- Finding the death per infection rate in the world;
...
-- Going to look at the total population, total vaccination and the percentage of those vaccinated per country
